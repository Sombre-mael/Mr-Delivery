...
export function Hero() {
  ...
  useGSAP(
    () => {
      ...
      timeline
        .fromTo(
          ".hero-copy > *",
          { autoAlpha: 0, y: isMobile ? 12 : 24 },
          { autoAlpha: 1, y: 0, duration: isMobile ? 0.68 : 0.72, stagger: isMobile ? 0.045 : 0.08, clearProps: "opacity,visibility,transform", immediateRender: false },
        )
        .fromTo(
          ".hero-delivery-motion",
          { autoAlpha: 0, y: isMobile ? 8 : 18 },
          { autoAlpha: 1, y: 0, duration: isMobile ? 0.52 : 0.55, clearProps: "opacity,visibility,transform", immediateRender: false },
          "-=0.34",
        )
        .fromTo(
          ".hero-stat",
          { autoAlpha: 0, y: isMobile ? 8 : 18 },
          { autoAlpha: 1, y: 0, duration: isMobile ? 0.42 : 0.48, stagger: isMobile ? 0.04 : 0.06, clearProps: "opacity,visibility,transform", immediateRender: false },
          "-=0.3",
        )
      ...
      if (!isMobile) {
        timeline.fromTo(
          ...
          { autoAlpha: 1, scale: 1, duration: 0.8, clearProps: "opacity,visibility,transform", immediateRender: false },
          0.35,
        );
      }
  ...
  return (
    <section
      ...
      className="relative px-4 pb-10 pt-24 sm:px-6 sm:pb-14 sm:pt-28 lg:px-8 lg:pb-20 lg:pt-32"
    >
      <div className="mx-auto grid max-w-7xl items-center gap-10 lg:grid-cols-[1.02fr_0.98fr]">
        <div className="hero-copy max-w-3xl">
          ...
          <div className="hero-delivery-motion mt-5 sm:mt-8">
            <DeliveryMotion compact />
          </div>

          <div className="mt-5 grid grid-cols-3 gap-2 sm:mt-8 sm:gap-3">
            {stats.map((stat) => (
              <div key={stat.label} className="rounded-lg border border-ink/8 bg-white/80 p-2.5 shadow-sm sm:p-4">
